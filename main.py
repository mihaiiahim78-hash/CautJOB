...
def calculeaza_compatibilitate_precisa(text_cv, job, lang):
    procent = 25 
    if not text_cv:
        return procent, "Introducere de bază. Încarcă fișierul CV."
...
async def pagina_principala_get(request: Request, q: str = "", locatie: str = "Bucuresti", tip: str = "Toate", lang: str = "ro", msg: str = "", page: int = 1):
    ...
    # MATEMATICĂ PAGINARE STRICTĂ (15 REZULTATE PER PAGINĂ)
    ...
    context = {
        "request": request, "joburi": joburi_paginate, "q": q, "locatie": locatie, 
        "tip": tip, "lang": lang, "cv_incarcat": False, "msg": msg, 
        "toate_recenziile": Baza_Recenzii_Companii,
        "current_page": page, "total_pages": total_pagini, "total_items": total_elemente
    }
    return templates.TemplateResponse(request=request, name="index.html", context=context)

@app.post("/", response_class=HTMLResponse)
async def pagina_principala_post(
    request: Request, q: str = Form(""), locatie: str = Form("Bucuresti"), tip: str = Form("Toate"), lang: str = Form("ro"), cv_file: UploadFile = File(None), page: int = 1
):
    joburi_totale = cauta_joburi_externe(cuvant_cheie=q, locatie=locatie, tip_job=tip)
    text_cv = ""
    try:
    ...
    if text_cv:
        joburi_totale = sorted(joburi_totale, key=lambda k: k["match_score"], reverse=True)
    ...
    context = {
        "request": request, "joburi": joburi_paginate, "q": q, "locatie": locatie, 
        "tip": tip, "lang": lang, "cv_incarcat": bool(text_cv), "msg": "", "toate_recenziile": Baza_Recenzii_Companii,
        "current_page": page, "total_pages": total_pagini, "total_items": total_elemente
    }
    return templates.TemplateResponse(request=request, name="index.html", context=context)
...
async def adauga_recenzie_globala(companie: str = Form(...), autor: str = Form(...), mail: str = Form(...), telefon: str = Form(...), note_salariu: int = Form(...), note_mgm: int = Form(...), comentariu: str = Form(...), q: str = Form(""), locatie: str = Form("Bucuresti")):
    if not re.match(r"[^@]+@[^@]+\.[^@]+", mail) or not re.match(r"^[0-9]{10}$", telefon.strip()):
        return RedirectResponse(url=f"/?q={q}&locatie={locatie}&msg=invalid", status_code=303)
